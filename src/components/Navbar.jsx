...
export function DropdownItems({ closeDropdown, isScrolled }) {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef(null);
  const dispatch = useDispatch();
  const navigate = useNavigate();
    ...
    return (
      <div className="flex items-center gap-3" ref={dropdownRef}>
        <NavLink
          ...
          to="/login"
          className={`font-medium rounded-lg text-xs px-4 py-2 text-center me-2 mb-2 transition-all duration-200 focus:ring-4 focus:outline-none focus:ring-secondary-500/30 ${
            isScrolled
              ? "text-primary-900 bg-secondary-500 hover:bg-gradient-to-bl from-secondary-900 to-secondary-500"
              : "text-primary-900 font-semibold text-xl border border-primary-900 hover:border-secondary-500 bg-transparent hover:bg-secondary-500/20 hover:text-secondary-500"
          }`}>
          Login
        </NavLink>
      </div>
  ...
  return (
    <div className="relative mid:ml-2" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 focus:outline-none">
        <div className="w-6 h-6 rounded-full overflow-hidden border-2 border-neutral-50">
          {profile?.image ? (
            <img
              src={`${profile?.image}`}
              alt={userInfo.username}
            ...
            <FaUserCircle className="w-full h-full text-neutral-500" />
          )}
        </div>
      ...
      {isOpen && (
        <div className="absolute  mid:left-0 right-0 mt-2 w-56 bg-white rounded-lg shadow-lg py-1 z-50 transform opacity-100 scale-100 transition-all duration-200">
          ...
          <NavLink
            to={dashboardPath}
            className={({ isActive }) =>
              `block px-4 py-2 text-sm text-neutral-500 hover:bg-neutral-50 hover:text-secondary-500 transition-colors duration-150 ${
                isActive ? "bg-neutral-50 text-secondary-500" : ""
              }`
            }
            onClick={() => {
              setIsOpen(false);
          ...
          <button
            ...
            className="w-full px-4 py-2 text-sm text-neutral-500 hover:bg-neutral-50 hover:text-secondary-500 transition-colors duration-150 flex justify-center items-center gap-2">
            <span>Logout</span>
            <HiOutlineLogout className="w-4 h-4" />
          </button>
        </div>
...
import MuiAlert from "@mui/material/Alert";
import CredulenLogo2 from "../assets/CredulenLogo2.png";
import CredulenLogo1 from "../assets/CredulenLogo1.png";
import CredulenLogo from "../assets/CredulenLogo.png";
import {
  NotificationBanner,
  LayoutWrapper,
...
const DropdownMenu = ({ title, items, closeDropdown }) => {
  ...
  return (
    <div
      ...
      onMouseLeave={handleMouseLeave}>
      <div className=" mid:mb-2  flex items-center cursor-pointer">
        <span className="block py-2 px-3 rounded md:hover:bg-transparent hover:text-secondary-500 md:p-0 transition-all duration-300">
          {title}
        </span>
        <svg
          className={`w-2.5 h-2.5 ms-2 transform transition-transform duration-300 ${
      ...
      <div
        className={`absolute z-10 right-[-3rem] mid:left-0 text-neutral-900 rounded-lg shadow-lg w-44 mt-2 bg-white border transform transition-all duration-200 ease-in-out ${
          isOpen
            ? "opacity-100 translate-y-0 visible"
            : "opacity-0 translate-y-1 invisible"
        }`}>
          ...
          {items.map((item, index) => (
            <li key={index} className="relative group/item">
              <NavLink
                ...
                target={item.external ? "_blank" : undefined}
                className={({ isActive }) =>
                  `block px-4 py-2 hover:bg-neutral-50 hover:text-secondary-500 transition-colors duration-200 ${
                    isActive
                      ? "text-secondary-500 bg-neutral-50"
                      : "text-neutral-500"
                  }`
                }
                onClick={() => {
                ...
                <span className="relative z-10">{item.label}</span>
                <div className="absolute inset-0 bg-neutral-50 transform scale-x-0 group-hover/item:scale-x-100 transition-transform duration-200 origin-left" />
              </NavLink>
            </li>
          ))}
        </ul>
...
const Navbar = () => {
  ...
  const navbarClass = `fixed w-full z-20 top-0 start-0 transition-all duration-300 ease-in-out ${
    isScrolled
      ? "bg-primary-900 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-600 text-white"
      : "bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-600"
  }`;
    ...
    <>
      <nav className={navbarClass}>
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-1">
          <Link
            to="/"
            className="flex cursor-pointer items-center space-x-3 rtl:space-x-reverse transition-all duration-300">
            <img
              src={isScrolled ? CredulenLogo1 : CredulenLogo}
          ...
          <button
            onClick={toggleMenu}
            className={`inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-neutral-50 focus:outline-none focus:ring-2 focus:ring-neutral-500/20`}
            aria-controls="navbar-sticky"
            aria-expanded={isMenuOpen}>
            <span className="sr-only">Open main menu</span>
            <svg
            ...
            <ul
              ...
              <li>
                <NavLink
                  ...
                  to="/"
                  className={({ isActive }) =>
                    `block py-2 px-3 mid:mb-2 rounded hover:bg-neutral-50 md:hover:bg-transparent hover:text-secondary-500 md:p-0 ${
                      isActive
                        ? "mid:bg-secondary-500 mid:text-primary-900"
                        : ""
                    }`
                  }
                  end>
              ...
              <li>
                <NavLink
                  ...
                  to="/blog"
                  className={({ isActive }) =>
                    `block mid:mb-2  py-2 px-3 rounded hover:bg-neutral-50 md:hover:bg-transparent hover:text-secondary-500 md:p-0 ${
                      isActive
                        ? "text-secondary-500 mid:bg-secondary-500 mid:text-primary-900"
                        : ""
                    }`
                  }
                  end>
