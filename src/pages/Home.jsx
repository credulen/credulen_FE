...
const IntelligentHomepage = () => {
  ...
  const fetchWebinars = useCallback(async () => {
    try {
      ...
      const past = webinars
        ...
        .sort((a, b) => new Date(b.date) - new Date(a.date)); // Sort past events by most recent
  ...
  return (
    <>
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-6 pt-8 lg:pt-0">
              ...
              <motion.div
                ...
                {/* Course Title */}
                <div className="relative">
                  <h2 className="text-2xl md:text-3xl font-semibold mb-2 text-primary-900">
                    Courses &
                  </h2>
                  <div className="absolute -bottom-2 left-0 w-24">
                    <svg viewBox="0 0 172 12" className="w-full">
              ...
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.8, delay: 0.4 }}
                className="text-neutral-600 max-w-xl text-base md:text-lg">
                Project-based training and Scenario-based Innovation Consulting
                to master the skills & value creation in a rapidly changing
                world
              </motion.p>
              ...
              <motion.div
                ...
                className="flex flex-wrap gap-4">
                <Link to="/solutions/training_School">
                  <button className="relative px-6 md:px-8 py-2 md:py-3 bg-secondary-500 text-primary-900 rounded-full font-semibold focus:ring-4 focus:outline-none focus:ring-secondary-500/30 transition-all duration-300 group overflow-hidden hover:scale-105 hover:shadow-lg">
                    <span className="relative z-10 transition-opacity duration-200 group-hover:opacity-0">
                      Courses
                    </span>
                    <div className="absolute top-0 left-1/2 w-1/2 h-full bg-primary-900 -translate-x-full transition-transform duration-500 ease-out group-hover:translate-x-0" />
                    <div className="absolute top-0 right-1/2 w-1/2 h-full bg-primary-900 translate-x-full transition-transform duration-500 ease-out group-hover:translate-x-0" />
                    <span className="absolute inset-0 flex items-center justify-center text-secondary-500 font-semibold opacity-0 transition-all duration-300 delay-200 group-hover:opacity-100 z-20">
                      📚 Courses
                    </span>
                  </button>
                </Link>
                <Link to="/solutions/consulting_Services">
                  <button className="relative px-6 md:px-8 py-2 md:py-3 bg-primary-900 text-white rounded-full font-semibold focus:ring-4 focus:outline-none focus:ring-primary-500/30 transition-all duration-300 group overflow-hidden hover:scale-105 hover:shadow-lg">
                    <span className="relative z-10 transition-opacity duration-200 group-hover:opacity-0">
                      Solutions
                    </span>
                    <div className="absolute top-1/2 left-0 w-full h-1/2 bg-secondary-500 -translate-y-full transition-transform duration-500 ease-out group-hover:translate-y-0" />
                    <div className="absolute bottom-1/2 left-0 w-full h-1/2 bg-secondary-500 translate-y-full transition-transform duration-500 ease-out group-hover:translate-y-0" />
                    <span className="absolute inset-0 flex items-center justify-center text-primary-900 font-semibold opacity-0 transition-all duration-300 delay-200 group-hover:opacity-100 z-20">
                      ⚡ Solutions
                    </span>
                  </button>
                </Link>
              ...
              <motion.div
                ...
                className="text-sm md:text-base">
                <Link target="blank" to="https://www.youtube.com/@credulen">
                  <span className="text-neutral-600 hover:text-primary-900 transform hover:scale-105 transition-all duration-300">
                    Watch Our Previous Masterclasses here
                  </span>
                </Link>
              </motion.div>
              ...
              <div className="relative w-full">
                <motion.div
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.2, delay: 1.1 }}
                    className="absolute -bottom-4 left-0 right-0 bg-white/90 backdrop-blur-sm p-4 mx-4 rounded-lg shadow-lg border border-primary-100">
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div>
                      ...
                      <div className="border-x border-primary-200">
                        <div className="text-xl md:text-2xl lg:text-3xl font-bold text-secondary-500">
                          7+
                        </div>
                        <div className="text-xs md:text-sm text-neutral-500">
                          Courses
      ...
      {/* Hero Section Ends */}
      {/* Main Content */}
      <div className="min-h-screen bg-gradient-to-b from-primary-50 to-neutral-50 text-primary-900 px-6">
        <motion.main
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
            ...
            <div className="relative bg-transparent py-16">
              <div className="max-w-6xl mx-auto px-4">
                <motion.div
                  ...
                  <TeaserSection
                    title="Join our upcoming webinars"
                    posts={memoizedUpcomingEvents}
                    pastPosts={memoizedPastEvents}
          ...
          {/* Mission Section */}
          <div className="bg-neutral-50 py-16 flex justify-center items-center min-h-screen">
            <div className="max-w-6xl mx-auto px-4 text-center lg:max-w-4xl">
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <MissionCard
                  className="text-center mx-auto flex flex-col items-center"
                  icon={<Rocket className="w-12 h-12 text-primary-500 mb-4" />}
                  title="Our Mission"
                  content="At Credulen, our mission is to empower individuals and organizations to harness emerging technologies as tools to unlock creativity, supercharge productivity, and enhance human authenticity like never before."
                />
                <MissionCard
                  icon={<Star className="w-12 h-12 text-secondary-500" />}
                  title="Our Approach"
                  content="We deliver value through a combination of hands-on training and strategic consulting, designed to address the unique challenges of our rapidly evolving world."
                />
              </motion.div>
...
const SkillCard = ({ icon, title, description, linkTo }) => {
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.05 }}
      className="bg-primary-500 p-8 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border border-primary-100">
      <div className="flex items-center mb-4">
        {icon}
        <h2 className="text-xl font-bold ml-4 text-white">{title}</h2>
      </div>
      <p className="text-primary-200 mb-6">{description}</p>
      <motion.button
        whileHover={{ x: 5 }}
        className="flex items-center text-secondary-500 hover:text-secondary-600 font-semibold"
        onClick={handleLearnMore}>
        Learn More{" "}
        <ArrowRight className="w-5 h-5 ml-2 hover:text-secondary-600" />
      </motion.button>
    </motion.div>
  );
};
...
const TeaserSection = ({
  ...
  // Use upcoming events if available, otherwise use up to 3 past events
  const displayItems = posts?.length > 0 ? posts : pastPosts?.slice(0, 3);
  const displayTitle = posts?.length > 0 ? title : "Explore our past webinars";

  return (
    <div className="bg-white shadow-primary-200 p-8 rounded-lg shadow-lg border border-primary-100">
      <h2 className="text-2xl text-primary-900 font-bold mb-6 flex items-center">
        {icon}
        ...
        <ul className="space-y-4">
          {displayItems.map((item) => (
            <motion.li
              key={item.id}
              whileHover={{ x: 5 }}
              className="border-b border-primary-200 pb-2">
              <Link
                to={getItemLink(item)}
                className="text-primary-900 hover:text-secondary-500 transition-colors duration-300">
                <h3 className="font-semibold">{item.title}</h3>
      ...
      <motion.button
        whileHover={{ scale: 1.05 }}
        className="mt-6 bg-primary-900 text-white px-3 py-1 rounded-md font-semibold hover:bg-primary-500 hover:text-white focus:ring-4 focus:outline-none focus:ring-primary-500/30 transition-all duration-300"
        onClick={handleViewAll}>
        View All
      </motion.button>
    </div>
...
const MissionCard = ({ icon, title, content, className }) => {
  return (
    <motion.div
      whileHover={{ scale: 1.05 }}
      className={`text-neutral-900 p-8 rounded-lg hover:shadow-sm transition-all duration-300 flex flex-col items-center text-center ${className}`}>
      <div className="flex flex-col items-center mb-4 text-neutral-900">
        {icon}
        <h2 className="text-2xl font-bold mt-4">{title}</h2>
      </div>
      <p className="leading-relaxed text-neutral-900">{content}</p>
    </motion.div>
  );
};
